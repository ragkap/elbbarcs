...
function tile(letter, x, y, size) {
  ...
  return `
    <g transform="translate(${x},${y})">
      ...
      <text x="${size * 0.32}" y="${size * 0.7}" font-family="Inter, sans-serif"
            font-size="${fontMain}" font-weight="800" fill="#2a1d05">${escapeXml(letter.toUpperCase())}</text>
      <text x="${size * 0.72}" y="${size * 0.88}" font-family="Inter, sans-serif"
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <text x="${W / 2}" y="${tileY + tileSize + 80}" font-family="Inter, sans-serif"
        font-size="38" font-weight="500" fill="#b8c4ad" text-anchor="middle">
    two players · two phones · one love for words
  </text>
  <text x="${W / 2}" y="${tileY + tileSize + 140}" font-family="Inter, sans-serif"
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <rect width="${W}" height="${H}" fill="url(#bg)"/>
  ${wordRow(word, startX, tileY, tileSize)}
  <text x="${W / 2}" y="${tileY + tileSize + 90}" font-family="Inter, sans-serif"
        font-size="48" font-weight="700" fill="#f4ede0" text-anchor="middle">
    ${displayName} has invited you to play
  </text>
  <text x="${W / 2}" y="${tileY + tileSize + 160}" font-family="Inter, sans-serif"
...
function renderPNG(svg) {
  ...
  // Bundle Inter so server containers without system fonts (Railway/nixpacks)
  // still render the text. loadSystemFonts:false keeps startup fast.
